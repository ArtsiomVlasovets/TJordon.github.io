...
down.onmouseout=function(){
		...
		function fTime(){
			...
			home.style.minHeight = (homeHeight-100)+'px'; 
		}
...
$('.call-btn').magnificPopup({
  ...
  midClick: true // allow opening popup on middle mouse click. Always set it to true if you don't provide alternative source.
		...
		for (let i of portfolioItem) {
			...
			i.addEventListener('mouseout', overOut);
		} 
		...
		function over(){
			...
			//this.getElementsByClassName('callIn')[0].style.opacity='1';
			//$('.callIn').animate({'opacity':'1'},600);
		}	
			function overOut(){
			//this.getElementsByClassName('callIn')[0].style.opacity='0';
			this.getElementsByClassName('descrTravel')[0].classList.toggle('hide');
			this.getElementsByClassName('callIn')[0].classList.toggle('hide');
